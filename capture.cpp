...
// 获取当前屏幕图片;
void Capture::loadBackgroundPixmap()
{
...
// 当前鼠标坐标是否在选取的矩形区域内;
bool Capture::isPressPointInSelectRect(QPoint mousePressPoint)
{
...
QRect Capture::getRect(const QPoint &beginPoint, const QPoint &endPoint)
{
...
// 获取当前鼠标位于哪一个拖拽顶点;
StretchRectState Capture::getStrethRectState(QPoint point)
{
...
// 设置鼠标停在拖拽定点处的样式;
void Capture::setStretchCursorStyle(StretchRectState stretchRectState)
{
...
void Capture::drawSelectRectInfo()
{
    int posX, posY;
    ...
    QPushButton button_ok(this);
    button_ok.setText("这是一个按钮");
    button_ok.move(QPoint(topLeftPoint.x() + (SELECT_RECT_INFO_WIDTH - fontWidth) / 2, topLeftPoint.y() + 14));
    button_ok.resize(100,100);
}



// 绘制鼠标拖拽时选区矩形的右下顶点的放大图;
void Capture::drawEndPointImage()
{
    int posX, posY;
